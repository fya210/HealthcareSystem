...
export default class AuthController {
  static async register(req, res, next) {
    try {
      ...
      const addSessionResponse = await SessionDAO.addSession(
        userInfo.username,
        sessionStartTime
      );
      ...
      // console.log(userObj);
      res.json({ authToken: authToken, ...userObj.toShortJson() });
    } catch (err) {
  ...
  static async updatePassword(req, res, next) {
    try {
      ...
      const passwordMatched = await userObj.comparePassword(
      ...
      const updateResponse = await UserDAO.updateUser(user.username, {
  ...
  static async authorizeSession(req, res, next) {
    if (
      ...
      !req.path.startsWith("/auth/signin")
    ) {
      try {
        ...
        res
          ...
          .json({ message: "Failed to authorize user session." });
        next(err);
      }
    }
    next();

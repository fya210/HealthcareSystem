...
function DefaultAppointmentItem(props) {
  return (
    <WidgetListItem>
      <FluidContainer>
        <Row>
          <Col className='md-font-sm text-center text-muted'>
    ...
    async function load() {
      try {
        ...
        const response = await axios.get(
          `http://localhost:4090/api/appointments/${props.appointment.id}`,
          {
            headers: {
        ...
        console.error(
          `Failed to load appointment- ${props.appointment.id}. ${err}`
        );
      } finally {
        setIsLoading(false);
      }
  ...
  useEffect(() => {
    async function initialize() {
      try {
        ...
        const page = 0;
        const response = await axios.get(
          `http://localhost:4090/api/appointments?view=waiting&page=${page}&limit=${state.limit}`,
          {
            headers: {
              Authorization: `Bearer ${props.session.authToken}`,
            },
